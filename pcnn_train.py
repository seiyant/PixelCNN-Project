...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    ...
    for batch_idx, (model_input, categories) in enumerate(tqdm(data_loader)): #add categories
        ...
        label_tensor = torch.tensor(label_int, dtype=torch.long).to(device) #convert string to int label

        model_output = model(model_input, class_label=label_tensor) #pass label tensor
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if (epoch + 1) == args.max_epochs or epoch % args.sampling_interval == 0: #ensure last measurement
            print('......sampling......')
